/** ---------------------------------------------------------------------------
 * \file        EcLogging.cpp
 * \brief       EtherCAT Example Logging
...
#if (EC_OS == EC_OS_FREERTOS) || (EC_OS == EC_OS_VXWORKS)\
    || (EC_OS == EC_OS_TIRTOS) || (EC_OS == EC_OS_XILINX_STANDALONE)\
    || (EC_OS == EC_OS_ECOS) || (EC_OS == EC_OS_UC3) || (EC_OS == EC_OS_UCOS)
...
#if (defined INCLUDE_EC_MASTER) || (defined INCLUDE_EC_MONITOR) || (defined INCLUDE_EC_SIMULATOR) || (defined INCLUDE_EC_EAP) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
...
#if (defined INCLUDE_EC_MASTER) || (defined INCLUDE_EC_MONITOR) || (defined INCLUDE_EC_SIMULATOR) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
    m_dwPerfMeasNumOf = 0;
...
struct _MSG_BUFFER_DESC* CAtEmLogging::AddLogBuffer(
    ...
    EC_T_BOOL   bPrintFile,         /* [in]  print message in file? */
#endif
    ...
    )
{
    ...
    {
        OsSnprintf(szLogFileNameTmp, sizeof(szLogFileNameTmp)-1, "%s%s%d", EC_LOGGING_PATH_PREFIX, szLogFilename, dwInstanceId);
    }
#else
...
#if (defined INCLUDE_EC_MASTER) || (defined INCLUDE_EC_MONITOR) || (defined INCLUDE_EC_SIMULATOR) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
    OsSafeFree(m_aPerfMeasVal);
    OsSafeFree(m_aPerfMeasInfo);
    m_dwPerfMeasNumOf = 0;
#endif
}
...
)
{
    ...
    OsSafeStrcpy(pMsgBufferDesc->szMsgLogFileName, szMsgLogFileName);
    pMsgBufferDesc->szMsgLogFileName[MAX_PATH_LEN - 1] = '\0';
    OsSafeStrcpy(pMsgBufferDesc->szMsgLogFileExt,  szMsgLogFileExt);
    ...
    OsSafeStrcpy(pMsgBufferDesc->szLogName, szBufferName);
#endif /* INCLUDE_FILE_LOGGING */
...
Exit:
    if (!bOk)
    {
        OsSafeFree(pMsgBufferDesc->paMsg);
        OsSafeFree(pchMsgBuffer);
    }
    return bOk;
}
...
)
{
    ...
    if (pMsgBufferDesc->bIsInitialized)
    {
        ...
        {
            OsPrintf("Store unsaved messages in '%s' message/logging buffer...\n", pMsgBufferDesc->szLogName);
            oTimeout.Start(3000);
        ...
        if (EC_NULL != pMsgBufferDesc->paMsg)
        {
...
)
{
    ...
    if (0 == pMsgBufferDesc->dwNumMsgs)
    {
        /* message buffer disabled, drop message */
        dwRes = EC_E_NOERROR;
        goto Exit;
    }
...
EC_T_VOID CAtEmLogging::PrintPerfMeasInternal(struct _EC_T_LOG_CONTEXT* pContext, EC_T_DWORD dwNumOf, EC_T_PERF_MEAS_VAL* aPerfMeasVal, EC_T_PERF_MEAS_INFO* aPerfMeasInfo)
{
    ...
    for (dwPerfMeasIdx = 0; dwPerfMeasIdx < dwNumOf; ++dwPerfMeasIdx)
    {
        ...
        {
            LogMsg(pContext, EC_LOG_LEVEL_ANY, "PerfMsmt '%s' (min/avg/max) [us]: %4d.%d/%4d.%d/%4d.%d\n",
...
#if (defined INCLUDE_EC_MASTER) || (defined INCLUDE_EC_MONITOR) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
...
static EC_T_VOID StripStr(EC_T_CHAR* szDst, EC_T_CHAR* szSrc)
{
    ...
    OsSafeStrncpy(szDst, szSrc + dwStartPos, dwEndPos - dwStartPos + 1);
}
...
EC_T_DWORD CAtEmLogging::PrintHistogramAsCsv(EC_T_DWORD dwInstanceId, EC_T_VOID* pvPerfMeasApp)
{
    ...
    {
#if (defined INCLUDE_FILE_LOGGING)
        m_pHistMsgBufferDesc = AddLogBuffer(dwInstanceId, 0, DEFAULT_LOG_MSG_BUFFER_SIZE, EC_FALSE, (EC_T_CHAR*)"Histogram", (EC_T_CHAR*)"hist", (EC_T_CHAR*)"csv", bLogFileEnb, EC_FALSE, EC_FALSE);
#else
...
#elif (defined INCLUDE_EC_MASTER) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
    ...
    dwRetVal = EC_E_NOERROR;
Exit:
    OsSafeFree(aPerfMeasInfo);
    if (EC_NULL != aHistogram)
    {
        for (dwMeasIdx = 0; dwMeasIdx < (dwMeasNum + dwMeasAppNum); dwMeasIdx++)
        {
            OsSafeFree(aHistogram[dwMeasIdx].aBins);
        }
    }
    OsSafeFree(aHistogram);
    OsSafeFree(szLineBuf);
...
#if (defined INCLUDE_FILE_LOGGING)
    FILE*         pFileHandle                 = pMsgBufferDesc->pfMsgFile;
...
EC_T_DWORD CAtEmLogging::SetLogDir(EC_T_CHAR* szLogDir)
{
    ...
    OsSafeStrcpy(m_pchLogDir, szLogDir);
...
/********************************************************************************/
/** \brief Initialize additional logging features
 *
 * \return EC_E_NOERROR on success, error code otherwise.
 */
EC_T_DWORD InitLoggingFeatures(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
#if (defined INCLUDE_PCAP_RECORDER) || (defined INCLUDE_FRAME_MIRROR)
    EC_T_DWORD dwRetVal = EC_E_ERROR;
    EC_T_DWORD dwRes = EC_E_NOERROR;
    ...
    {
        pAppContext->pPcapRecorder = EC_NEW(CPcapRecorder());
        if (EC_NULL == pAppContext->pPcapRecorder)
        {
            dwRetVal = EC_E_NOMEMORY;
        ...
        dwRes = pAppContext->pPcapRecorder->StartPcapRecorder(pAppContext->dwInstanceId, pAppParms->dwPcapRecorderBufferFrameCnt, pAppParms->szPcapRecorderFileprefix);
...
#if (defined PCAP_RECORDER_ALL_ESC_PORTS)
    esLogFrameEnableAtSlavePort(pAppContext->dwInstanceId, 0, ESC_PORT_A, CFrameLogMultiplexer::StaticFrameHandler, CFrameLogMultiplexer::GetInstance(pAppContext->dwInstanceId));
    esLogFrameEnableAtSlavePort(pAppContext->dwInstanceId, 0, ESC_PORT_B, CFrameLogMultiplexer::StaticFrameHandler, CFrameLogMultiplexer::GetInstance(pAppContext->dwInstanceId));
    esLogFrameEnableAtSlavePort(pAppContext->dwInstanceId, 0, ESC_PORT_C, CFrameLogMultiplexer::StaticFrameHandler, CFrameLogMultiplexer::GetInstance(pAppContext->dwInstanceId));
    esLogFrameEnableAtSlavePort(pAppContext->dwInstanceId, 0, ESC_PORT_D, CFrameLogMultiplexer::StaticFrameHandler, CFrameLogMultiplexer::GetInstance(pAppContext->dwInstanceId));
#endif
#endif /* INCLUDE_PCAP_RECORDER */

#if (defined INCLUDE_FRAME_MIRROR)
    if (EC_NULL != pAppParms->pLinkParmsMirror)
    {
        pAppContext->pFrameMirror = EC_NEW(CFrameMirror());
        if (EC_NULL == pAppContext->pFrameMirror)
        {
            dwRetVal = EC_E_NOMEMORY;
            EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "ERROR: Creating FrameMirror failed: %s (0x%lx)\n", pAppContext->dwInstanceId, ecatGetText(dwRetVal), dwRetVal));
            goto Exit;
        }
        dwRes = pAppContext->pFrameMirror->InitInstance(pAppParms->pLinkParmsMirror);
        if (dwRes != EC_E_NOERROR)
        {
            dwRetVal = dwRes;
            EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "ERROR: Initialize FrameMirror failed: %s (0x%lx)\n", pAppContext->dwInstanceId, ecatGetText(dwRes), dwRes));
            goto Exit;
        }
        dwRes = pAppContext->pFrameMirror->StartFrameMirror(pAppContext->dwInstanceId);
        if (dwRes != EC_E_NOERROR)
        {
            dwRetVal = dwRes;
            EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "ERROR: Start FrameMirror failed: %s (0x%lx)\n", pAppContext->dwInstanceId, ecatGetText(dwRes), dwRes));
            goto Exit;
        }
    }
...
/********************************************************************************/
/** \brief De-Initialize additional logging features
 *
 * \return EC_E_NOERROR on success, error code otherwise.
 */
EC_T_VOID DeinitLoggingFeatures(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
    EC_UNREFPARM(pAppContext);
#if (defined INCLUDE_PCAP_RECORDER)
    SafeDelete(pAppContext->pPcapRecorder);
#endif
#if (defined INCLUDE_FRAME_MIRROR)
    SafeDelete(pAppContext->pFrameMirror);
#endif
}
...
{
    EC_T_UINT64 qwNow = 0;
    OsSystemTimeGet(&qwNow);

#if (defined INCLUDE_PCAP_RECORDER_OS_PERF_MEAS)    
    if (0 == m_dwFirstFrameTimeStampSecOffset)
    {
        EC_T_UINT64 qwSecSinceEpoch = 946684800ULL + qwNow / 1000000000ULL;
        
        m_dwFirstFrameTimeStampSecOffset = (EC_T_DWORD)qwSecSinceEpoch;
    }
...
#else
    *pqwTimestamp = 946684800ULL * 1000000000ULL + qwNow;
#endif
}
...
#if (defined INCLUDE_FRAME_MIRROR)
CFrameMirror::CFrameMirror()
{
    m_dwInstanceId = INVALID_CLIENT_ID;
    m_pvLinkInstance = EC_NULL;
    m_pLinkDrvDesc = EC_NULL;
    OsMemset(&m_LinkFrameDesc, 0, sizeof(EC_T_LINK_FRAMEDESC));
}

CFrameMirror::~CFrameMirror()
{
    StopFrameMirror();
    if (EC_NULL != m_pLinkDrvDesc)
    {
        m_pLinkDrvDesc->pfEcLinkClose(m_pvLinkInstance);
        OsFree(m_pLinkDrvDesc);
    }
}

EC_T_DWORD CFrameMirror::InitInstance(EC_T_LINK_PARMS* pLinkParms)
{
    EC_T_DWORD dwRes = EC_E_ERROR;
    EC_T_DWORD dwRetVal = EC_E_NOERROR;
    EC_PF_LLREGISTER pfEmllRegister = EC_NULL;

    pfEmllRegister = OsGetLinkLayerRegFunc(pLinkParms->szDriverIdent, pLinkParms->szLoadPath);
    if (EC_NULL == pfEmllRegister)
    {
        dwRetVal = EC_E_OPENFAILED;
        goto Exit;
    }

    m_pLinkDrvDesc = (EC_T_LINK_DRV_DESC*)OsMalloc(sizeof(EC_T_LINK_DRV_DESC));
    if (EC_NULL == m_pLinkDrvDesc)
    {
        dwRetVal = EC_E_NOMEMORY;
        goto Exit;
    }
    OsMemset(m_pLinkDrvDesc, 0, sizeof(EC_T_LINK_DRV_DESC));

    m_pLinkDrvDesc->dwValidationPattern = LINK_LAYER_DRV_DESC_PATTERN;
    m_pLinkDrvDesc->dwInterfaceVersion = LINK_LAYER_DRV_DESC_VERSION;

    dwRes = pfEmllRegister(m_pLinkDrvDesc, sizeof(EC_T_LINK_DRV_DESC));
    if (dwRes != EC_E_NOERROR)
    {
        dwRetVal = dwRes;
        goto Exit;
    ...
    pLinkParms->dwLinkSpeed = 1000;
    dwRes = m_pLinkDrvDesc->pfEcLinkOpen(pLinkParms, EC_NULL, EC_NULL, EC_NULL, &m_pvLinkInstance);
    if ((dwRes != EC_E_NOERROR) && (dwRes != EC_E_LINK_DISCONNECTED))
    {
        dwRetVal = dwRes;
        goto Exit;
    }
...
Exit:
    if (EC_E_NOERROR != dwRetVal)
    {
        OsSafeFree(m_pLinkDrvDesc);
    }
    return dwRetVal;
}

EC_T_DWORD CFrameMirror::StartFrameMirror(EC_T_DWORD dwInstanceId)
{
    m_dwInstanceId = dwInstanceId;
    CFrameLogMultiplexer::AddFrameLogger(m_dwInstanceId, this, CFrameMirror::LogFrameStatic);
    return EC_E_NOERROR;
}

EC_T_DWORD CFrameMirror::StopFrameMirror(EC_T_VOID)
{
    if (INVALID_CLIENT_ID != m_dwInstanceId)
    {
        CFrameLogMultiplexer::RemoveFrameLogger(m_dwInstanceId, this, CFrameMirror::LogFrameStatic);
        m_dwInstanceId = INVALID_CLIENT_ID;
    }
    return EC_E_NOERROR;
}

EC_T_VOID CFrameMirror::LogFrame(EC_T_DWORD dwLogFlags, EC_T_DWORD dwFrameSize, EC_T_BYTE* pbyFrame)
{
    EC_UNREFPARM(dwLogFlags);
    if (EC_NULL != m_pLinkDrvDesc)
    {
        m_LinkFrameDesc.pbyFrame = pbyFrame;
        m_LinkFrameDesc.dwSize = dwFrameSize;
        m_pLinkDrvDesc->pfEcLinkSendFrame(m_pvLinkInstance, &m_LinkFrameDesc);
    }
}
