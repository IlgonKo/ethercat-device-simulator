 ...
 * \brief       EtherCAT Master generic notification handler
...
#if (defined INCLUDE_RAS_SERVER) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
/*****************************************************************************/
/**
                                       ...
                                       )
{
    ...
    case EC_RAS_NOTIFY_EXCEPTION:              /* ERROR RAS | 6 */
        {
            EC_PT_RAS_EXCEPTIONDESC pExceptionDescSerialized = (EC_PT_RAS_EXCEPTIONDESC)pParms->pbyInBuf;

            EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "Exception occurred in thread '%s'! Cookie: 0x%lx",
                pExceptionDescSerialized->szThreadName, EC_GET_FRM_DWORD(&pExceptionDescSerialized->dwCookie)));
        } break;
    default:
        {
...
#endif /* INCLUDE_RAS_SERVER || INCLUDE_EC_MASTER_RAS_CLIENT */
...
EC_T_DWORD CEmNotification::SetNotificationEnabled(EC_T_DWORD dwCode, EC_T_DWORD dwEnabled)
{
    ...
    if (EC_NOTIFICATION_ENABLED == dwEnabled)
    {
        ...
        dwRes = emIoCtl(m_dwMasterInstance, EC_IOCTL_SET_NOTIFICATION_ENABLED, &oNotificationCtlParms, sizeof(EC_T_SET_NOTIFICATION_ENABLED_PARMS), EC_NULL, 0, EC_NULL);
        if (EC_E_NOERROR != dwRes)
        {
