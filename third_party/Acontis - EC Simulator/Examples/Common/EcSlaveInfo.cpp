...
EC_T_BOOL PrintSlaveInfos(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
...
#if (defined INCLUDE_EC_MASTER) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
    EC_T_DWORD dwNumConnectedSlaves = emGetNumConnectedSlaves(pAppContext->dwInstanceId);
#elif (defined INCLUDE_EC_MONITOR) || (defined INCLUDE_EC_MONITOR_RAS_CLIENT)
    EC_T_DWORD dwNumConnectedSlaves = emonGetNumConnectedSlaves(pAppContext->dwInstanceId);
#elif (defined INCLUDE_EC_SIMULATOR) || (defined INCLUDE_EC_SIMULATOR_RAS_CLIENT)
    ...
    dwRes = emonGetBusSlaveInfo(pAppContext->dwInstanceId, EC_FALSE, wAutoIncAddress, &oBusSlaveInfo);
        ...
        EcLogMsg(EC_LOG_LEVEL_INFO, (pEcLogContext, EC_LOG_LEVEL_INFO, "Cfg Device Name.....: %s\n", oCfgSlaveInfo.abyDeviceName));
...
EC_T_VOID PrintVarInfos(T_EC_DEMO_APP_CONTEXT* pAppContext, EC_T_PROCESS_VAR_INFO_EX* apVarInfos, EC_T_DWORD dwVarCnt)
{
#define MAX_DATATYPENAME_LEN 63
    ...
    for (EC_T_DWORD dwVarIdx = 0; dwVarIdx < dwVarCnt; dwVarIdx++)
    {
        EC_T_PROCESS_VAR_INFO_EX* pVarInfo = &apVarInfos[dwVarIdx];
 ...
 * \return EC_TRUE on success, EC_FALSE otherwise.
 */
EC_T_BOOL PrintSlavesProcVarInfos(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
    EC_T_BOOL  bRetVal = EC_FALSE;
    EC_T_DWORD dwRes = EC_E_NOERROR;
    EC_T_DWORD dwProcVarInfoNumOf = 0;
    EC_T_DWORD dwProcVarInfoNumOfRead = 0;
    EC_T_PROCESS_VAR_INFO_EX* apVarInfos = EC_NULL;

    /* get all INPUT and OUTPUT variables count of all configured slaves */
#if (defined INCLUDE_EC_MASTER) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
        emGetProcessVarInfoNumOf(pAppContext->dwInstanceId, eVarDirection_All, eVarSource_AllSlaves, EC_TRUE, 0, &dwProcVarInfoNumOf);
#elif (defined INCLUDE_EC_MONITOR) || (defined INCLUDE_EC_MONITOR_RAS_CLIENT)
        emonGetProcessVarInfoNumOf(pAppContext->dwInstanceId, eVarDirection_All, eVarSource_AllSlaves, EC_TRUE, 0, &dwProcVarInfoNumOf);
#elif (defined INCLUDE_EC_SIMULATOR) || (defined INCLUDE_EC_SIMULATOR_RAS_CLIENT)
        esGetProcessVarInfoNumOf(pAppContext->dwInstanceId, eVarDirection_All, eVarSource_AllSlaves, EC_TRUE, 0, &dwProcVarInfoNumOf);
#endif

    if (dwProcVarInfoNumOf > 0)
    {
        /* allocate buffer */
        apVarInfos = (EC_T_PROCESS_VAR_INFO_EX*)OsMalloc(dwProcVarInfoNumOf * sizeof(EC_T_PROCESS_VAR_INFO_EX));
        ...
        OsMemset(apVarInfos, 0, dwProcVarInfoNumOf * sizeof(EC_T_PROCESS_VAR_INFO_EX));

        /* get all INPUT and OUTPUT variables of all configured slaves */
#if (defined INCLUDE_EC_MASTER) || (defined INCLUDE_EC_MASTER_RAS_CLIENT)
        dwRes = emGetProcessVarInfoEx(pAppContext->dwInstanceId, eVarDirection_All, eVarSource_AllSlaves, EC_TRUE, 0, apVarInfos, dwProcVarInfoNumOf, &dwProcVarInfoNumOfRead);
#elif (defined INCLUDE_EC_MONITOR) || (defined INCLUDE_EC_MONITOR_RAS_CLIENT)
        dwRes = emonGetProcessVarInfoEx(pAppContext->dwInstanceId, eVarDirection_All, eVarSource_AllSlaves, EC_TRUE, 0, apVarInfos, dwProcVarInfoNumOf, &dwProcVarInfoNumOfRead);
#elif (defined INCLUDE_EC_SIMULATOR) || (defined INCLUDE_EC_SIMULATOR_RAS_CLIENT)
        dwRes = esGetProcessVarInfoEx(pAppContext->dwInstanceId, eVarDirection_All, eVarSource_AllSlaves, EC_TRUE, 0, apVarInfos, dwProcVarInfoNumOf, &dwProcVarInfoNumOfRead);
#endif
        if (EC_E_NOERROR != dwRes)
        {
            bRetVal = EC_FALSE;
        ...
        PrintVarInfos(pAppContext, apVarInfos, dwProcVarInfoNumOfRead);

        OsSafeFree(apVarInfos);
    }
    bRetVal = EC_TRUE;
Exit:
    return bRetVal;
...
EC_T_BOOL PrintAllSlavesProcVarInfos(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
    ...
    bRetVal = PrintSlavesProcVarInfos(pAppContext);
